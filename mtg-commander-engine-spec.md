...
**Still open:** a way to measure whether the grounding is actually helping now that it's live (log declined-then-XMage-grounded outcomes separately from plain-declined ones — e.g. via `parsed_cards.source`/a new column — so this can be judged on real evidence from actual bulk-parser and gameplay runs, not just the design reasoning above).
