...
## Phase 3a: XMage as a static reference corpus for declined cases (built 2026-09-11)
...
**Concrete findings, verified live against `magefree/mage` on GitHub:**
- **License:** MIT (`LICENSE.txt`, confirmed via the GitHub license API). No copyleft concern; feeding a card's source as ephemeral LLM context (not redistributing the repo) is unambiguously fine, but a `NOTICE`/attribution line crediting the XMage project is still the right thing to add wherever this ships.
- **Structure:** one Java file per unique card name at `Mage.Sets/src/mage/cards/<first-letter-lowercase>/<NormalizedName>.java` — e.g. `cards/k/KodamasReach.java`, `cards/g/GixYawgmothPraetor.java`. The normalization is mechanical and reproducible from a Scryfall name: strip apostrophes/commas/periods, remove spaces, keep the rest verbatim including case (`"Kodama's Reach"` → `KodamasReach`, `"Gix, Yawgmoth Praetor"` → `GixYawgmothPraetor`). Confirmed against several real names; a normalizer function would need real-world validation against edge cases (split cards like `"Fire // Ice"`, DFCs, cards with digits or non-ASCII letters) before being trusted blindly — treat unmatched names as "no XMage source found," never guess a filename.
- **Scale:** 32,270 card implementation files, ~65 MB of source text total (via the repo's full recursive git tree, confirmed non-truncated). Comfortably small enough to live in a SQLite table (`cardDb.ts` already has one for the card catalog) or as flat files — no meaningful storage concern.
- **Quality, spot-checked against Gix, Yawgmoth Praetor** (a card this project hand-implemented from scratch this same week, so there's a direct point of comparison): XMage's `GixYawgmothPraetorTriggeredAbility.checkTrigger` independently encodes the exact same logic this project derived by hand — checks `game.getOpponents(controllerId).contains(damagedEvent.getTargetId())` (the "damaged player is an opponent of the WATCHER, not the attacker" board-wide check) and resolves the "may pay life, draw a card" choice against the attacking creature's controller, not the permanent's own controller. This is strong evidence the corpus is accurate enough to be worth grounding an LLM on, not just plausible-looking. Each file's Java comments and `staticText`/`setTriggerPhrase` string literals also reproduce the card's oracle text broken into per-ability granules verbatim — useful on its own even before an LLM reads the surrounding logic.
- **A real nuance to design around:** most cards are fully self-contained in one file (as Gix is — its triggered ability and both effect classes all live in the same file), but some reference a shared, reusable ability/effect class defined in a different file/package (e.g. a common "draw a card" or "create a Treasure token" helper). A naive single-file lookup will occasionally miss context that a shared class would have carried — acceptable as a first cut (still strictly better than today's plain decline), but worth flagging rather than promising perfect grounding.

**Ingestion — built 2026-09-11:**
1. `scripts/ingest-xmage-cards.mjs` (`npm run xmage:ingest`; `--no-fetch` to skip the git step and just re-index what's already checked out; `--validate` to also print the hit-rate report below). First run does `git clone --depth 1 --filter=blob:none --sparse` into `data/xmage-source` (gitignored — external source, not this repo's own content), then `git sparse-checkout set Mage.Sets/src/mage/cards`; later runs `git pull --ff-only` to refresh (best-effort — a failed pull logs a warning and continues indexing whatever's already on disk, rather than aborting). Far cheaper than paginating the GitHub REST API (which silently caps directory listings at 1000 entries per call) or fetching 32k individual raw-file requests.
2. Indexed into a new `xmage_cards` table in `cardDb.ts`'s existing SQLite file (`data/cards.db`), keyed by `xmage_key` — always derived straight from the real file's own name (`path.basename(file, ".java").toLowerCase()`), never guessed, so the ingested index can't be wrong about what it actually found. `xmageKeyForCardName` (exported from `src/lib/cardDb.ts`, tested in `cardDb.test.ts`) derives the SAME key from a Scryfall card name, for the future lookup direction — see its own doc comment for the exact normalization rule and its one documented gap (a true split card's combined name, e.g. `"Fire // Ice"`, doesn't match XMage's combined filename `FireIce.java`, since the key is derived from the front face's name only — right for the much more common modal-DFC/transform case, wrong for true splits). `replaceAllXMageCards`/`getXMageCardByKey`/`getXMageCardByName`/`getXMageIngestStats` round out the table's accessors.
3. **Real, measured outcome of a live ingestion run:** 32,270 files ingested (~67 MB on disk including the sparse checkout's own `.git`), and `--validate` reports **30,102/30,574 (98.5%)** of this project's own `data/cards.db` card names resolve to an ingested XMage file — confirming the scoping estimate above with an actual run, not just a sample. The ~1.5% misses are a mix of genuine XMage coverage gaps (a card it hasn't implemented — e.g. `Aminatou, Veil Piercer`, `Arcum's Whistle`) and the documented split-card gap (`Alive // Well`).

**Lookup/grounding — built 2026-09-11:** `src/lib/xmageGrounding.ts`'s `xmageReferenceBlurb` formats a `getXMageCardByName` hit into LLM-ready context (capped at 6000 characters — comfortably past the ingested corpus's measured p99 of ~6.5 KB, so truncation is rare; explicitly tells the model this is read-only grounding from a DIFFERENT engine, not instructions to follow, and to keep answering with only this system's own fixed step vocabulary), shared by both fallback sites — but wired in with two DIFFERENT strategies, because the two callers have different cost profiles:
- **`cardParser.ts`'s `requestCardParse`** (offline bulk parser, runs over ~30k cards, latency is cheap): tries the plain attempt first; only when the result actually declined does it look up and retry ONCE more with the XMage source appended — so the large majority of cards that already parse fine never pay for the extra context at all. Falls back to the plain (declined) result if the grounded retry itself fails validation twice, or if XMage has no source for that card.
- **`primitiveActionPlan.ts`'s `requestPrimitiveActionPlan`** (live, per-event, runtime fallback — a real game is waiting on this): looks up and includes the XMage source PROACTIVELY, in the one call this function makes, rather than reactively after a decline — a second live Ollama round-trip would add real latency to actual gameplay, so the cost is paid upfront instead (this function is already only reached once the deterministic parsers have given up on a card, so the extra prompt cost is reasonable there).

Both callers' schemas, validation, and the function-calling-only contract are otherwise completely unchanged — this only ever adds grounding context to the prompt, never a new way for the LLM to act or a new output shape to validate.

Attribution — done 2026-09-11: the repo root's `NOTICE` file credits the XMage project, names exactly what this project does with its source (local ingestion + LLM grounding context only, never redistributed), and reproduces the full MIT license text/copyright line as verified against `magefree/mage`'s own `LICENSE.txt`.

**Observability — built 2026-09-11:** `parsed_cards.used_xmage_grounding` (a new column, migrated onto the existing table via an idempotent `PRAGMA table_info` check + `ALTER TABLE` in `getCardDb()` — the first schema migration this codebase has needed, since every table before this one was brand new rather than altered) records whether the SAVED result actually came from `requestCardParse`'s grounded retry, not just whether a reference existed. `getParseStats()`'s new `xmageGrounding` field (`{ totalUsed, byStatus }`) answers "did grounding help" directly from real data — `byStatus.ok` is cards grounding got past a plain decline, `byStatus.declined` is cards where a reference existed but didn't help. `scripts/parse-cards.mjs`'s own console output surfaces the same split per run (`grounded(ok=N declined=M)` in the live tally, plus a one-line summary at the end). Live gameplay gets the same visibility a different way: `primitiveActionPlan.ts`'s `requestPrimitiveActionPlan` return (and `/api/rules/primitive-plan`'s response) carries `usedXMageGrounding`, and `AppFlow.tsx`'s event log appends "— grounded via XMage reference" to a resolved primitive-plan message (both the fresh-fetch and client-side-cache-hit paths) whenever it was used.
